...
export default function SliderInputBase({name, title, value, updateTrainingParams, min, max, step} : any) {
  return (
    <Box margin={'5px'} padding={'5px'} background={'#33334444'} borderRadius={'8px'}>
        ...
        <Slider aria-label='slider-ex-2' value={value} min={min} max={max} step={step} name={name} onChange={(e) => updateTrainingParams(e, name)}>
            <SliderTrack>
                <SliderFilledTrack />
            </SliderTrack>
            <SliderThumb />
